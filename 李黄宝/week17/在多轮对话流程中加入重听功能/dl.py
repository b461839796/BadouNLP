...
class DialogueSystem:
    def __init__(self):
        self.reset_texts = ["重听", "我没听懂", "没听明白", "请重新描述下", "我没理解", "什么？", "我没听清", "听不懂"]
        self.load()
    ...
    def intent_judge(self, memory):
        ...
        max_score = -1
        hit_node = None
        is_reset = self.rest_judge(query, memory)
        if is_reset:
            memory['reset'] = True
            return memory
    ...
    def rest_judge(self, query, memory):
        #是否重听，即用户输入与上次输入相同
        max_score = -1
        for node in self.reset_texts:
            score = self.calucate_sentence_score(query, node)
