...
class DialogueSystem:
    ...
    def load(self):
        self.all_node_info = {}
        self.load_scenario("D:\worksapce\\ai_workspace\\nlp20\week17 对话系统\scenario\scenario-买衣服.json")
        self.load_scenario("D:\worksapce\\ai_workspace\\nlp20\week17 对话系统\scenario\scenario-看电影.json")
        self.load_slot_templet("D:\worksapce\\ai_workspace\\nlp20\week17 对话系统\scenario\slot_fitting_templet.xlsx")

    def load_scenario(self, file):
        self.all_node_info["node0"] = {"intent":["请再说一遍","你刚才说什么？"]} # 增加重做节点
    ...
    def nlu(self, memory):
        memory = self.intent_judge(memory) # 意图识别
        memory = self.slot_filling(memory) # 槽位填充
        return memory
    ...
    def intent_judge(self, memory):
        ...
        for node in memory["available_nodes"]:
            ...
            if score > max_score:
                max_score = score
                hit_node = node
        if "hit_node" in memory and memory["hit_node"]:  # 添加键存在性检查
            memory["history_node"] = memory["hit_node"]
    ...
    def slot_filling(self, memory):
        if(memory["hit_node"] == "node0"):
            memory["hit_node"] = memory["history_node"]
            return memory
        #槽位填充
    ...
    def dpo(self, memory):
        ...
        memory["available_nodes"].append("node0") # 增加重做节点
        return memory
    ...
    def run(self, query, memory):
        ...
        memory = self.dst(memory) # 状态跟踪
        memory = self.dpo(memory) # 策略选择
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes":["scenario-买衣服_node1","scenario-看电影_node1","node0"]} #用户状态
    while True:
        query = input("请输入：")
        # query = "你好"    
