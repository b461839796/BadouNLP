...
class DialogueSystem:
    ...
    def is_repeat(self,query):
        repeat_words=["再说一遍","重复","重听","你说什么","什么"]
    ...
    def run(self, query, memory):
        ...
        if self.is_repeat(query):
            memory["response"] = memory.get("last_response","请问有什么可以帮您？")
            return memory
        memory["query"] = query
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes":["scenario-买衣服_node1"],
              "last_response":"" #用来储存上一次回复
              } #用户状态,"scenario-看电影_node1"
    while True:
        query = input("请输入：")
        #query = "你好"    
