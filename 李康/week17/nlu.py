...
class DialogueSystem:
    ...
    def nlg(self, memory):
        #根据policy执行反问或回答repeat时回答
        if memory["policy"] == "reply" or memory["policy"] == "repeat":
            hit_node = memory["hit_node"]
            node_info = self.all_node_info[hit_node]
    ...
    def set_repeat_policy(self, memory):
    ...
    def is_repeat(self, query):
        for note in self.repeat_word:
            score = self.calucate_sentence_score(query, note)
            if score >= 0.5:
                return True
        return False        
    def run(self, query, memory):
        ...
        memory: 用户状态
        '''
        if self.is_repeat(query):
            return self.set_repeat_policy(memory)
        memory["query"] = query
