...
class DialogueSystem:
    def __init__(self):
        self.load()
        self.sentence = ["请重复一遍","没听清，再说一次"]
    
    def load(self):
        self.all_node_info = {}
        self.load_scenario(r"F:\1\八斗精品班\第十七周 对话系统\week17 对话系统\scenario\scenario-买衣服.json")
        #self.load_scenario("scenario-看电影.json")
        self.load_slot_templet(r"F:\1\八斗精品班\第十七周 对话系统\week17 对话系统\scenario\slot_fitting_templet.xlsx")
    ...
    def run(self, query, memory):
        ...
        memory: 用户状态
        '''
        if memory["available_nodes"] in ["scenario-买衣服_node1","scenario-看电影_node1"]:
            memory["query"] = query
            ...
            memory["query"] = query
            memory = self.chongfa(memory)
            if memory["cd_score"] > 0.5:
                return memory
