...
class DialogueSystem:
    ...
    def load(self):
        self.all_node_info = {}
        self.load_scenario(r"D:\AI课程学习\week17 对话系统\week17 对话系统\scenario\scenario-买衣服.json")
        #self.load_scenario("scenario-看电影.json")
        self.load_slot_templet(r"D:\AI课程学习\week17 对话系统\week17 对话系统\scenario\slot_fitting_templet.xlsx")
    ...
    def load_slot_templet(self, file):
        ...
        for i in range(len(self.slot_templet)):
            ...
            self.slot_info[slot]['values'] = values#这里将表格里的第一列，也就是槽位当做了键值，其他当做了值，值是一个字典
      
    def nlu(self, memory):
        memory = self.intent_judge(memory)#我先要判断当前是那个节点，问的问题是那个节点对应的问题intent
        ...
        for slot in node_info.get('slot', []):
            if slot not in memory:
                ...
                if re.search(slot_values, query):
                    memory[slot] = re.search(slot_values, query).group()#正则匹配，会匹配询问里时候有和槽位模版对应的value里一样的文字，有的话
                    #就会返回这个匹配到的文字，只会返回匹配到的第一个选项
                    #这个函数的作用就是根据询问，先将能够提取到的有用信息都先提取了
        return memory
    ...
    def dst(self, memory):
        ...
        for s in slot:
            if s not in memory:
                memory["require_slot"] = s#查看当前是否需要补充信息，缺少槽位就需要让使用者补充
                return memory
    ...
    def nlg(self, memory):
        ...
        hit_node = memory["hit_node"]
        node_info = self.all_node_info[hit_node]
        self.all_node_info["scenario-买衣服_node5"] = node_info
        self.all_node_info["scenario-买衣服_node5"]["intent"]=["我没听清楚"]
        return memory
    ...
    def fill_in_slot(self, template, memory):
        ...
        node_info = self.all_node_info[node]
        for slot in node_info.get("slot", []):
            template = template.replace(slot, memory[slot])#寻找template中是否有和slot一样的文字，有的话就用memory[slot]替换
        return template
