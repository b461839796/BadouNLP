...
class DialogueSystem:
    ...
    def intent_judge(self, memory):
        #意图识别，匹配当前可以访问的节点
        query = memory['query']
        # memory = {'require_repeat':False,'last_response':None}
        if re.search(r'(重复|再说一遍|没听清|重听|说啥呢)',query):
            memory['require_repeat'] = True
            return memory
        max_score = -1
        hit_node = None
    ...
    def calucate_node_score(self, query, node): #两个字符串作相似度计算
        #节点意图打分，算和intent相似度
